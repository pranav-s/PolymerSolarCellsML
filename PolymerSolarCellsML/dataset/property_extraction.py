...
from PSC_data_extractor.dataset.dataset_base_classes import EntityList, PropertyValuePair, RecordProcessor
...
class PropertyExtractor(RecordProcessor):
    def __init__(self, grouped_spans=None, text=None, property_mentions=None, abbreviation_pairs=None, print_spans=False, logger=None):
        ...
        property_metadata_file = '../../metadata/property_metadata.json'
    ...
    def property_extraction(self, sentence, labels):
        ...
        if (
            category_counts.get('PROP_VALUE', 0) >= 2
            and category_counts.get('PROP_NAME', 0) >= 2
            and any(span.text == 'respectively' for span in sentence)
        ):
